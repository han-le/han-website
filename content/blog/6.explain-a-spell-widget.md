...
Our form now is good but basically we are not able to interact with it yet. We will add an event listener to the "Explain" button and handle the logic with a function in the client script. In Service Portal widgets, we use AngularJS syntax **`ng-click`** instead of the standard HTML **`onclick`**:
